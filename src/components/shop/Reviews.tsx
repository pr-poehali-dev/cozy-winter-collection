...
export default function Reviews() {
  return (
    <section id="reviews" className="py-10 px-6 md:px-8 bg-white relative overflow-hidden">
      ...
      <div className="absolute top-8 right-12 text-xl opacity-15">💫</div>
      <div className="absolute bottom-6 left-16 text-xl opacity-15">🤍</div>
      <div className="absolute bottom-4 right-8 text-2xl opacity-20">💗</div>
      
      <div className="max-w-3xl mx-auto relative">
        <h2 className="text-2xl md:text-3xl font-light text-primary text-center mb-2">отзывы ✨</h2>
        ...
        <div className="space-y-4 mb-8">
          <div className="p-4 rounded-2xl space-y-2 bg-gradient-to-br from-orange-50/20 to-amber-50/20">
            <div className="flex gap-1 justify-center text-amber-400">
              {[...Array(5)].map((_, i) => (
                <Icon key={i} name="Star" size={12} className="fill-current" strokeWidth={0} />
              ))}
            </div>
            <p className="text-muted-foreground leading-relaxed font-light text-xs text-center">
